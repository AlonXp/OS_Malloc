...
size_t threasold = 128*1024;
#define MAX 4*1024*1024*sizeof(long)
void pirnt2();
void pirnt();
void pirnt3();
/*function for List: */
...
void* mmap_alloc(size_t size, int t){
...
void* mmap_alloc_huge(size_t size, int t){
    void* new_alloc = mmap(NULL,size + sizeof(MallocMetadata),PROT_READ | PROT_WRITE,  MAP_ANONYMOUS | MAP_PRIVATE | MAP_HUGETLB, -1, 0);
    if(new_alloc == MAP_FAILED)
        return nullptr;
    MallocMetadata* new_meta = (MallocMetadata*)new_alloc;
    new_meta->size = size;
...
void* mmap_srealloc(size_t size, MallocMetadata* prev, int t){
    if(size == prev->size)
        return prev + 1;
    ...
    void* new_mmap;
    if(prev->origin == 1 && size >= 1000 * 1000 * 4)
    	new_mmap = mmap_alloc_huge(size, 1);
    else if (prev->origin == 2 && size > 1000 * 1000 * 2)
    	new_mmap = mmap_alloc_huge(size, 2);
    else
	new_mmap = mmap_alloc(size, t);
    if(prev->size > size)
...
/*function: */
void* smalloc_wrapper(size_t size, int c){
    size_t new_size = Alignment_size(size);
    if(new_size == 0 || new_size > 100000000)
        return nullptr;
    if(new_size >= 1000 * 1000 * 4 && c == 1 && new_size >= threasold)
        return mmap_alloc_huge(new_size, 1);
    if(new_size >= threasold)
        return mmap_alloc(new_size, c);
    ...
    if(c == 1)
	current_meta->origin = 1;
    if(c == 2)
	current_meta->origin = 2;
    if(tail_malloc == nullptr){
...
void* smalloc(size_t size){
    return smalloc_wrapper(size, 1);
}
void* scalloc(size_t num, size_t size){
    size_t new_size = Alignment_size(size*num);
    if(new_size == 0 || num == 0 || new_size > 100000000)
        return nullptr;
    if(size > 1000 * 1000 * 2 && size >= threasold) {
        void* check = mmap_alloc_huge(new_size, 2);
        return memset(check, 0 , new_size);
    }
    void* ret = smalloc_wrapper(new_size, 2);
    if(ret == nullptr)
        return nullptr;
...
void sfree(void* p){
    ...
    if(temp->is_free)
        return;
    if(temp->size > threasold && temp->size <= MAX) {
        threasold = temp->size;
    }
    if(temp->mmap){
...
void* srealloc(void* oldp, size_t size){
    ...
    if(new_size == 0 || new_size > 100000000)
        return nullptr;
    if(oldp == nullptr){
        return smalloc_wrapper(new_size, 1);
    }
    MallocMetadata* old = (MallocMetadata*)oldp - 1;
    if(old->mmap)
        return mmap_srealloc(new_size,old, old->origin);
    if(old->size >= size){
    ...
    void* new_adress = smalloc_wrapper(new_size, -1);
...
void pirnt3(){
    std::cout << "mmap list:" << std::endl;
    MallocMetadata* temp = mmap_list_head;
    while (temp != nullptr){
        std::cout << "adress:" << (size_t)temp << std::endl;
        std::cout << "size:" << temp->size << std::endl;
        temp = temp->next;
